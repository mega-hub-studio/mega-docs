...
| `POST /api/chat` | `{question, scope?, fresh?, history?}` → SSE: `token` · `citations` · `done{cached,in,out}`, or `error`. `history` is `[{q,a}]`, oldest first — the thread a follow-up continues |
