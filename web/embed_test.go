...
func TestReleaseNotesAreGenerated(t *testing.T) {
	...
	// The history is generated by the same parser over the same git log, so it is held to the
	// same standard: a version that is a real tag, and a sha behind every line. This is what
	// stops the modal's older sections from becoming the hand-written changelog rule 25
	// refuses — the newest release having provenance would not save them.
	for _, past := range r.History {
		if !regexp.MustCompile(`^v\d+\.\d+\.\d+$`).MatchString(past.Version) {
			t.Errorf("history entry %q is not a tag of the form v0.13.0", past.Version)
		}
		if len(past.Notes) == 0 {
			t.Errorf("history entry %s carries no notes — an empty range is skipped, not emitted",
				past.Version)
		}
		for _, n := range past.Notes {
			if !sha.MatchString(n.Commit) {
				t.Errorf("history %s: note %q has no commit behind it — a line nobody can trace "+
					"to a sha is a line somebody typed", past.Version, n.Subject)
			}
		}
	}
