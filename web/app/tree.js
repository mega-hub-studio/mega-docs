/* ══ tree.js — the corpus as a tree, and the scope picked from it ══════════════
   One folder of documents is one question's worth of context. This is the control that
   ...
   `<nes-tree>` is the design system's own element — expand/collapse, single selection,
   full keyboard nav, ARIA tree semantics. It takes its data as a child JSON script and
   builds its DOM once, in connectedCallback, so this component *replaces* the element
   when the corpus changes rather than mutating it. That is the whole reason the markup is
   created here instead of in index.html.
   ═══════════════════════════════════════════════════════════════════════════ */
 ...
 * few hundred documents opened all at once is a wall, and the level above is the scope
 * most people want anyway. The exception is the path down to the active scope, which is
 * opened so a restored scope is visible where it lives rather than highlighted inside a
 * collapsed branch.
 *
 * @param {Doc[]} documents
...
export const CorpusTree = {
  name: "CorpusTree",
  // Inline template: the element and its JSON are built imperatively below, so all this
  // needs is somewhere to put them. (The pinned Vue global build ships the compiler, so a
  // string template costs no build step.)
  ...
  setup(props, { emit }) {
    const { onMounted, watch, useTemplateRef } = Vue;
    ...
    function build() {
      ...
      if (props.scope) tree.setAttribute("value", props.scope);
      const data = document.createElement("script");
      data.type = "application/json";
      data.textContent = JSON.stringify(treeNodes(props.documents, props.scope));
      ...
      tree.addEventListener("nes:change", (e) => emit("pick", e.detail.value || ""));
      host.value.replaceChildren(tree);
    }

    onMounted(build);

    // The corpus changes when a BA imports or confirms. Rebuilding is the only option —
    // the element ignores a mutated JSON payload — and it is cheap.
    watch(() => props.documents, build);

    // Reflect a scope cleared from elsewhere (the picker above the prompt). Only a
    // *different* value rebuilds: rebuilding on the value the tree itself just emitted
    // would drop the keyboard position mid-interaction.
    watch(
      () => props.scope,
      (now) => {
